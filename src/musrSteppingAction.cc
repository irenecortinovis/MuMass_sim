...
double musrSteppingAction::grid_transmission = 0.92;
...
void musrSteppingAction::UserSteppingAction(const G4Step* aStep)  { 
  ...
  if (aTrack->GetDefinition()) {
    ...
    if ((p_name == "mu+")||(p_name == "mu-")||(p_name == "Mu")||(p_name == "Mu2S"))  {
    ...
    if (((processName=="Decay")||(processName=="DecayWithSpin")) && ((p_name == "Mu") || (p_name == "Mu2S"))) {
      ...
      G4Track* new_Track = new G4Track(bound_electron, decay_track->GetGlobalTime(), decay_track->GetPosition());
      new_Track->SetParentID(decay_track->GetParentID()+1);
      //not sure how to set proper TrackID...
      new_Track->SetTrackID(0);
      G4EventManager::GetEventManager()->GetStackManager()->PushOneTrack(new_Track);
    ...
    {
     G4PropagatorInField* propagMgr = G4TransportationManager::GetTransportationManager()->GetPropagatorInField();
     propagMgr->SetLargestAcceptableStep(1*CLHEP::mm);
    }
   
    if(aTrack->GetVolume()->GetLogicalVolume()->GetName() == "log_QuenchingGrid")
    {
        G4double random_transmission = G4UniformRand();
        
        if(random_transmission > grid_transmission) //particle hits grid
        {
            //set kinetic energy to 0, so it decays on the spot there
            //only for mu+, mu-, Mu & Mu2S
            //all other particles go through without interference so far
