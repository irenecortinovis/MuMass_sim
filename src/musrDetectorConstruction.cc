...
#include "mumassMuFormation.hh" //included by Gianluca
...
G4VPhysicalVolume* musrDetectorConstruction::Construct()  {
  ...
  while (!feof(fSteeringFile)) {
	...
	if (strcmp(tmpString2,"tubs")==0){
    ...
    else if (strcmp(tmpString2,"BoxAllSidesCenteredHole")==0){
	  sscanf(&line[0],"%*s %*s %*s %s %lf %lf %lf %lf %lf %s %lf %lf %lf %s %s",
		 name,&x1,&x2,&x3,&x4,&x5,material,&posx,&posy,&posz,mothersName,rotMatrix);
	  sscanf(&line[0],"%*s %*s %*s %*s %*g %*g %*g %*g %*g %*s %*g %*g %*g %*s %*s %s %d %s",sensitiveDet,&volumeID,actualFieldName);
	  solidName+=name;
	  G4Box* solidCube     = new G4Box ("solidCube",x1*CLHEP::mm,x2*CLHEP::mm,x3*CLHEP::mm);
	  G4Tubs* solidTubeHole = new G4Tubs("solidTubeHole",0.*CLHEP::mm,x4*CLHEP::mm,x5*CLHEP::mm,0*CLHEP::deg,360*CLHEP::deg);
      G4RotationMatrix* roty = new G4RotationMatrix(G4ThreeVector(0.,1.,0.),90.*CLHEP::deg);
	  G4RotationMatrix* rotx = new G4RotationMatrix(G4ThreeVector(1.,0.,0.),90.*CLHEP::deg);
      G4ThreeVector nowhere( 0,0,0);

	  
	  G4VSolid *temp_solid =  new G4SubtractionSolid(solidName, solidCube, solidTubeHole);
      temp_solid =  new G4SubtractionSolid(solidName, temp_solid, solidTubeHole, rotx, nowhere);
      solid = new G4SubtractionSolid(solidName, temp_solid, solidTubeHole, roty, nowhere);
      

	}
      else if (strcmp(tmpString2,"BoxCenteredHole")==0){
	  sscanf(&line[0],"%*s %*s %*s %s %lf %lf %lf %lf %lf %s %lf %lf %lf %s %s",
		 name,&x1,&x2,&x3,&x4,&x5,material,&posx,&posy,&posz,mothersName,rotMatrix);
	  sscanf(&line[0],"%*s %*s %*s %*s %*g %*g %*g %*g %*g %*s %*g %*g %*g %*s %*s %s %d %s",sensitiveDet,&volumeID,actualFieldName);
	  solidName+=name;
	  G4Box* solidCube     = new G4Box ("solidCube",x1*CLHEP::mm,x2*CLHEP::mm,x3*CLHEP::mm);
	  G4Tubs* solidTubeHole = new G4Tubs("solidTubeHole",0.*CLHEP::mm,x4*CLHEP::mm,x5*CLHEP::mm,0*CLHEP::deg,360*CLHEP::deg);
      solid = new G4SubtractionSolid(solidName, solidCube, solidTubeHole);
      

	}
    
          else if (strcmp(tmpString2,"UShapedMesh")==0){
	  // Create first two mesh sheets with 
      // x1*x2 for front, x1*x4 the top/bottom, x3 is the thickness, x5 the transparency
	  ...
	  G4double roundingErr=0.01*CLHEP::mm;
      G4double lengthbox = (2.54*CLHEP::cm/x5);
	  G4Box* box1 = new G4Box("MeshFront",x1*CLHEP::mm,x2*CLHEP::mm,x3*CLHEP::mm);
	  G4Box* box2 = new G4Box("MeshTop",x1*CLHEP::mm,x4*CLHEP::mm,x3*CLHEP::mm);
      //G4Box* box3 = new G4Box("Holes",lengthbox/2-x3*CLHEP::mm,lengthbox/2-x3*CLHEP::mm,x3*CLHEP::mm);
    
      G4RotationMatrix* no_rot = new G4RotationMatrix(G4ThreeVector(0.,0.,0.),90.*CLHEP::deg);
      //G4ThreeVector* xtrans = new G4ThreeVector(0,0,0);
      G4double y_trans = 0;
      G4VSolid *temp_solid = box1->Clone();



      /*for(int j=0; j<2*x2*CLHEP::mm/lengthbox; j++){

          y_trans = -x2*CLHEP::mm+j*lengthbox;
          for(int i=0; i<2*x1*CLHEP::mm/lengthbox; i++){
              
            G4ThreeVector xtrans(-x1*CLHEP::mm+i*(lengthbox),y_trans,0);
            G4VSolid *new_temp_solid = temp_solid->Clone(); 
            G4SubtractionSolid temp_solid(solidName, new_temp_solid, box3, no_rot, xtrans);}
            
        }

      y_trans = 0;
      G4VSolid *temp_solid2 = box2->Clone();
      //G4ThreeVector xtrans(0,0,0);

      for(int j=0; j<2*x4*CLHEP::mm/lengthbox; j++){
          
          y_trans = -x4*CLHEP::mm+j*lengthbox;
          for(int i=0; i<2*x1*CLHEP::mm/lengthbox; i++){
            G4ThreeVector xtransnew(-x1*CLHEP::mm+i*(lengthbox),y_trans,0);
            G4VSolid *new_temp_solid2 = temp_solid2->Clone();
            G4SubtractionSolid temp_solid2(solidName, new_temp_solid2, box3, no_rot, xtransnew);
                
          }
       }*/
        
      G4RotationMatrix *rot = new G4RotationMatrix(G4ThreeVector(1.,0.,0.),90.*CLHEP::deg);
      G4ThreeVector top( 0., x2*CLHEP::mm+x3, x4*CLHEP::mm);
      G4ThreeVector bottom( 0., -x2*CLHEP::mm+x3, x4*CLHEP::mm);


	  G4VSolid *temp_ushape =  new G4UnionSolid(solidName, box1, box2, rot, top);
      solid =  new G4UnionSolid(solidName, temp_ushape, box2, rot, bottom);


	}
          
    // Added by Irene - hexagonal crystals for BGOs
    else if (strcmp(tmpString2,"hexaCrystal")==0)
    {
        // Load parameters: x1: radius, x2: half length
        sscanf(&line[0],"%*s %*s %*s %s %lf %lf %s %lf %lf %lf %s %s",
        name,&x1,&x2,material,&posx,&posy,&posz,mothersName,rotMatrix);
        sscanf(&line[0],"%*s %*s %*s %*s %*g %*g %*s %*g %*g %*g %*s %*s %s %d %s",sensitiveDet,&volumeID,actualFieldName);
        // Build BGO solid
        const G4double r_inner_BGO[] = {0, 0};
        const G4double r_outer_BGO[] = {x1*CLHEP::mm, x1*CLHEP::mm};
        const G4double z_BGO[] = {-x2*CLHEP::mm, x2*CLHEP::mm};
        solidName+=name;
        solid = new G4Polyhedra(solidName,CLHEP::pi/6,2*CLHEP::pi,6,2,z_BGO,r_inner_BGO,r_outer_BGO);
    }
	...
	;  // processes are interpreded later in  musrPhysicsList.cc
      ...
      else if (strcmp(tmpString1,"SetMu2SProbability")==0){
	sscanf(&line[0],"%*s %*s %lf",&musrMuFormation::Mu2S_probability);
	cout << "Mu2S Probability = " << musrMuFormation::Mu2S_probability << endl;
      }
      else if (strcmp(tmpString1,"SetLandauLoss")==0){
	sscanf(&line[0],"%*s %*s %lf",&musrMuFormation::landauLoss);
	cout << "landauSigma = " << musrMuFormation::landauLoss << endl;
      }

      else if (strcmp(tmpString1,"SetMuFormation")==0){
	sscanf(&line[0],"%*s %*s %lf",&musrMuFormation::MuFormation);
	cout << "MuFormation = " << musrMuFormation::MuFormation << endl;
      }
	...
	cout << "landauSigma = " << musrMuEnergyLossLandau::landauSigma << endl;
      ...
      else if (strcmp(tmpString1,"SetTemperature_Silica")==0){
	sscanf(&line[0],"%*s %*s %lf",&mumassMuFormation::mu_temperature);
	cout << "Temperature of silica target = " << mumassMuFormation::mu_temperature<< endl;
      }
      //included by Gianluca, Laser Stuff
      else if (strcmp(tmpString1,"SetBetaRabi")==0){
	sscanf(&line[0],"%*s %*s %lf",&Prob_LaserExc::betarabi);
	cout << "Beta (Rabi) = " << Prob_LaserExc::betarabi<< endl;
  }
      else if (strcmp(tmpString1,"SetBetaIoni")==0){
	sscanf(&line[0],"%*s %*s %lf",&Prob_LaserExc::betaioni);
	cout << "Beta (Ionization) = " << Prob_LaserExc::betaioni<< endl;
      }
      else if (strcmp(tmpString1,"SetBetaStark")==0){
	sscanf(&line[0],"%*s %*s %lf",&Prob_LaserExc::betastark);
	cout << "Beta (Stark) = " << Prob_LaserExc::betastark<< endl;
      }
      else if (strcmp(tmpString1,"SetDecayGroundState")==0){
	sscanf(&line[0],"%*s %*s %lf",&Prob_LaserExc::gdecay);
	cout << "Decay Rate of Mu(1S) = " << Prob_LaserExc::gdecay<< endl;
      }
      else if (strcmp(tmpString1,"SetDecayExcitedState")==0){
	sscanf(&line[0],"%*s %*s %lf",&Prob_LaserExc::edecay);
	cout << "Decay Rate of Mu(2S) = " << Prob_LaserExc::edecay<< endl;
      }
      else if (strcmp(tmpString1,"SetLaserDetuning")==0){
	sscanf(&line[0],"%*s %*s %lf",&Prob_LaserExc::detuning);
	cout << "Laser Detuning = " << Prob_LaserExc::detuning<< endl;
      }
      else if (strcmp(tmpString1,"SetLaserTimeOffset")==0){
	sscanf(&line[0],"%*s %*s %lf",&Prob_LaserExc::LaserTimeOffset);
	cout << "Laser TimeOffset = " << Prob_LaserExc::LaserTimeOffset<< endl;
      }
      else if (strcmp(tmpString1,"SetLaserDistanceSource")==0){
	sscanf(&line[0],"%*s %*s %lf",&Prob_LaserExc::DistanceSourceLaserX);
	cout << "Laser Distance = " << Prob_LaserExc::DistanceSourceLaserX<< endl;
      }
      else if (strcmp(tmpString1,"SetLaserFWHM")==0){
	sscanf(&line[0],"%*s %*s %lf",&Prob_LaserExc::LaserFWHM);
	cout << "Laser FWHM = " << Prob_LaserExc::LaserFWHM<< endl;
      }
      else if (strcmp(tmpString1,"SetLaserLambda")==0){
	sscanf(&line[0],"%*s %*s %lf",&Prob_LaserExc::lambda);
	cout << "Laser Wavelength Lambda = " << Prob_LaserExc::lambda<< endl;
      }
      else if (strcmp(tmpString1,"SetLaserW0")==0){
	sscanf(&line[0],"%*s %*s %lf",&Prob_LaserExc::w0);
	cout << "Laser w0 = " << Prob_LaserExc::w0<< endl;
      }
      else if (strcmp(tmpString1,"SetLaserPower")==0){
	sscanf(&line[0],"%*s %*s %lf",&Prob_LaserExc::Epr);
	cout << "Laser power = " << Prob_LaserExc::Epr<< endl;
      }
      
    else if (strcmp(tmpString1,"SetLaserPosX")==0){
	sscanf(&line[0],"%*s %*s %lf",&Prob_LaserExc::LaserPos_X);
	cout << "Laser position X= " << Prob_LaserExc::LaserPos_X<< endl;
      }
      
    else if (strcmp(tmpString1,"SetLaserPosY")==0){
	sscanf(&line[0],"%*s %*s %lf",&Prob_LaserExc::LaserPos_Y);
	cout << "Laser position Y = " << Prob_LaserExc::LaserPos_Y<< endl;
      }
      
    else if (strcmp(tmpString1,"SetLaserPosZ")==0){
	sscanf(&line[0],"%*s %*s %lf",&Prob_LaserExc::LaserPos_Z);
	cout << "Laser position Z = " << Prob_LaserExc::LaserPos_Z<< endl;
      }
    else if (strcmp(tmpString1,"SetGridTransmission")==0){
	sscanf(&line[0],"%*s %*s %lf",&musrSteppingAction::grid_transmission);
    
	cout << "Quenching Grid Transmission = " << musrSteppingAction::grid_transmission<< endl;
      }
...
//--------- Material definitions ---------
 ...
 // Elements required for MCPglass
  ...
  G4Material* BGO = // BGO scintillating crystals
    new G4Material("BGO", density= 7.10*CLHEP::g/CLHEP::cm3, ncomponents=3);
  BGO->AddElement(O, natoms=12);
  BGO->AddElement(Ge, natoms= 3);
  BGO->AddElement(Bi, natoms= 4);
    
  
  G4Material* peek = // PEEK rods of MCPs
    new G4Material("PEEK", density=1.31*CLHEP::g/CLHEP::cm3, ncomponents=3);
  peek->AddElement(C, fractionmass=0.76);
  peek->AddElement(H, fractionmass=0.08);
  peek->AddElement(O, fractionmass=0.16);
