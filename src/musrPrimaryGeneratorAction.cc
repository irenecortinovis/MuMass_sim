...
musrPrimaryGeneratorAction::musrPrimaryGeneratorAction(
  ...
  :musrDetector(musrDC), x0(0), y0(0), z0(-10*CLHEP::cm), xSigma(0), ySigma(0), zSigma(0), 
   ...
   takePosFromHisto(false), takeMomFromHisto(false),takeMomANDPosFromHisto(false)
						      //, firstCall(true)
{
...
void musrPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  if (takeMuonsFromTurtleFile) {
    ...
    do {
      ...
      if(takePosFromHisto){
         x = pos_histo_x->GetRandom(); 
         y = pos_histo_y->GetRandom(); 
         z = pos_histo_z->GetRandom(); 
      }
      else{
  ...
  if (takeMomFromHisto){
          ...
          px = mom_histo_x->GetRandom();
          py = mom_histo_y->GetRandom();
          pz = mom_histo_z->GetRandom();
          p = pow(px*px+py*py+pz*pz,0.5);
  ...
  if (takeMomANDPosFromHisto){
      
      
      tree->GetEntry(CLHEP::RandFlat::shoot()*tree->GetEntries());
      //x, y, z, px, py, pz
...
void musrPrimaryGeneratorAction::swapTheAxisInTurtle(float& x_x, float& x_xprime ,float& y_y, float& y_yprime) {
  ...
  void musrPrimaryGeneratorAction::SetMomHisto(G4String MomFileName) {
  takeMomFromHisto = true;
  TFile *mom_file = new TFile(MomFileName.c_str());
  if (mom_file==NULL) {
    G4cout << "E R R O R :    Failed to open ROOT Momentum input file \"" << MomFileName
	   <<"\"."<< G4endl;
    G4cout << "S T O P    F O R C E D" << G4endl;
    exit(1);
  }
  else {G4cout << "ROOT Momentum input file  \"" << MomFileName <<"\" opened."<< G4endl;
        mom_histo_x = (TH1D*)mom_file->Get("mom_hist_x");
        mom_histo_y = (TH1D*)mom_file->Get("mom_hist_y");
        mom_histo_z = (TH1D*)mom_file->Get("mom_hist_z");}
}

  void musrPrimaryGeneratorAction::SetPosHisto(G4String PosFileName) {
  takePosFromHisto = true;
  TFile *pos_file = new TFile(PosFileName.c_str());
  if (pos_file==NULL) {
    G4cout << "E R R O R :    Failed to open ROOT Position input file \"" << PosFileName
	   <<"\"."<< G4endl;
    G4cout << "S T O P    F O R C E D" << G4endl;
    exit(1);
  }
  else {G4cout << "ROOT Position input file  \"" << PosFileName <<"\" opened."<< G4endl;
      pos_histo_x = (TH1D*)pos_file->Get("pos_hist_x");
      pos_histo_y = (TH1D*)pos_file->Get("pos_hist_y");
      pos_histo_z = (TH1D*)pos_file->Get("pos_hist_z");
  }

}

  void musrPrimaryGeneratorAction::SetMomANDPosHisto(G4String MomPosFileName) {
  takeMomANDPosFromHisto = true;
  TFile *mompos_file = new TFile(MomPosFileName.c_str());
  if (mompos_file==NULL) {
    G4cout << "E R R O R :    Failed to open ROOT Position AND Momentum input file \"" << MomPosFileName
	   <<"\"."<< G4endl;
    G4cout << "S T O P    F O R C E D" << G4endl;
    exit(1);
  }
  else {G4cout << "ROOT Position and Momentum input file  \"" << MomPosFileName <<"\" opened."<< G4endl;
       tree = (TTree*)mompos_file->Get("tree"); 
