...
Mu2SDecayChannel::Mu2SDecayChannel(const G4String& theParentName, 
				       G4double        theBR)
               :G4VDecayChannel("Muonium 2S Decay",1)
{
  // set names for daughter particles
  if (theParentName == "Mu2S") {
    SetBR(theBR);
    SetParent("Mu2S");
    ...
    if (GetVerboseLevel()>0) {
      G4cout << "Mu2SDecayChannel:: constructor :";
...
Mu2SDecayChannel::~Mu2SDecayChannel()
{
}

G4DecayProducts *Mu2SDecayChannel::DecayIt(G4double) 
{
...
do {
 ...
 // output message
  ...
  if (GetVerboseLevel()>1) {
    G4cout << "Mu2SDecayChannel::DecayIt ";
