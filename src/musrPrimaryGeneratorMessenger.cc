...
{ 
  ...
  //if this is set, a root file to a histogram is expected with a position profile
  setPosHistoCmd =new G4UIcmdWithAString("/gun/PosHisto",this);
  setPosHistoCmd->SetGuidance("Set the file name for the histogram with the positionprofile.");
  setPosHistoCmd->SetParameterName("mes_PosHisto",true);
  setPosHistoCmd->AvailableForStates(G4State_PreInit,G4State_Idle);
  
  //if this is set, a root file to a histogram is expected with a momentum profile
  setMomHistoCmd =new G4UIcmdWithAString("/gun/MomHisto",this);
  setMomHistoCmd->SetGuidance("Set the file name for the histogram with the momentum profile.");
  setMomHistoCmd->SetParameterName("mes_MomHisto",true);
  setMomHistoCmd->AvailableForStates(G4State_PreInit,G4State_Idle);
  
   //if this is set, a root file to a histogram is expected with a momentum and position profile
  setMomANDPosHistoCmd =new G4UIcmdWithAString("/gun/MomANDPosHisto",this);
...
{ 
  ...
  if( command == setMomHistoCmd)
    { musrAction->SetMomHisto(newValue);}
  if( command == setPosHistoCmd)
